...
describe("pause", () => {
  ...
  it("should emit Paused CPI event", async () => {
  ...
  it("should allow idempotent pause operations", async () => {
    await program.methods
      .pause()
      .accountsPartial({
        pauser: client.owner.publicKey,
    ...
    const gatewayMinterState = await program.account.gatewayMinter.fetch(
      client.pdas.gatewayMinter.publicKey
    );
    expect(gatewayMinterState.paused).to.equal(true);
  });

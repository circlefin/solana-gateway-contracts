...
describe("GatewayMinter: updatePauser", () => {
  let svm: LiteSVM;
  ...
  it("owner can set pauser and emits event", async () => {
    ...
    const state = await client.gatewayMinterProgram.account.gatewayMinter.fetch(
  ...
  it("owner can set pauser to same value (idempotent)", async () => {
    const acct1 = await client.gatewayMinterProgram.account.gatewayMinter.fetch(
      client.pdas.gatewayMinter.publicKey
    );
